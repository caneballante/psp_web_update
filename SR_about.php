<!DOCTYPE html>
<html lang="en"><!-- InstanceBegin template="/Templates/page-4-template.dwt" codeOutsideHTMLIsLocked="false" -->
<head>
<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<!-- InstanceBeginEditable name="doctitle" -->
<title>Puget Sound Partnership - Salmon Recovery Council Members</title>
<!-- InstanceEndEditable -->
<!-- Bootstrap -->
<link href="css/bootstrap.css" rel="stylesheet">
<link href="css/custom.css" rel="stylesheet" type="text/css">
<link href="css/custom-erika.css" rel="stylesheet" type="text/css">
<!-- loads the wf-loading class right away to minimize FOUT -->
<script>document.documentElement.className += ' wf-loading';</script>
<!-- Font PRENTON TYPEKIT -->
<script src="https://use.typekit.net/srt5jze.js"></script>
<script>try{Typekit.load({ async: true });}catch(e){}</script>
<!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
	<script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
	<script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->
<!-- InstanceBeginEditable name="head" -->
<script>
/*this variable is used to set the proper nav to active. It should to the order the nav item is in the list*/
  	navSelected = 8;
</script>
<!-- InstanceEndEditable -->
<!-- InstanceParam name="OptionalRegion1" type="boolean" value="true" -->
<!-- InstanceParam name="OptionalRegion2" type="boolean" value="false" -->
<!-- InstanceParam name="OptionalRegion3" type="boolean" value="true" -->
<!-- InstanceParam name="OptionalRegion4" type="boolean" value="false" -->
<!-- InstanceParam name="OptionalRegion5" type="boolean" value="true" -->
<!-- InstanceParam name="OptionalRegion6" type="boolean" value="true" -->
<!-- InstanceParam name="OptionalRegion7" type="boolean" value="false" -->
<!-- InstanceParam name="OptionalRegion8" type="boolean" value="false" -->
</head>
<body>
<?php include 'includes/modal-inc.html';?>
<!-- START IMAGE HEADER --> 
 <!-- InstanceBeginEditable name="overviewphoto" -->
<header class="overview-page-image-style overview-page-image4"> </header>
<!-- InstanceEndEditable -->  
<div class="container-fluid page-content padding-50-bottom">
	<div class="row">
		<div class="col-sm-3 padding-20-top"></div>
		<div class="col-sm-7 padding-20-top"> <!-- InstanceBeginEditable name="6col_header" -->
			<h1>ABOUT THE SALMON RECOVERY COUNCIL</h1>
			<p>&nbsp;</p>
			<!-- InstanceEndEditable --></div>
		<div class="col-sm-2 padding-20-top"></div>
	</div>
	
	<div class="row"> 
		<div class="col-sm-3">
			<div class="nav-leftside-custom">
				<ul class="nav nav-stacked nav-pills nav-leftside-custom padding-left-0 margin-10-top">
					<!-- InstanceBeginEditable name="left_nav" -->
				<?php include 'includes/ln-boards.html';?>
				<!-- InstanceEndEditable -->
				</ul>
			</div>
		</div>
		 
		<div class="col-sm-7 padding-20-top content-column"> <!-- InstanceBeginEditable name="6col_content" -->
			<p>The Puget Sound Salmon Recovery Council is a regional body of policy decision‐makers for salmon recovery. The role of the Council is to advise the Puget Sound Partnership&rsquo;s Leadership Council on decisions relating to salmon recovery and the implementation of the Puget Sound Chinook Recovery Plan. Recovery Council decisions accomplish the following:</p>
			<ul>
				<li>Set policy direction for implementation of the Puget Sound Chinook Recovery Plan</li>
				<li>Develop and direct strategic approaches to near‐term issues and actions</li>
				<li>Establish allocation policies for capital funding</li>
				<li>Hold others and itself accountable for the implementation of the Recovery Plan</li>
			</ul>
			<table  class="table">
				<tr>
					<td valign="top"><p><strong>Representing</strong></p></td>
					<td valign="top"><p><strong>Affiliation</strong></p></td>
					<td valign="top"><p><strong>Member</strong></p></td>
					<td valign="top"><p><strong>First Alternate</strong></p></td>
					<td valign="top"><p><strong>Second Alternate</strong></p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>WATERSHEDS</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Cities</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Counties</strong></p></td>
					<td valign="top"><p>Washington Association of Counties</p></td>
					<td valign="top"><p><strong>Ken Dahlstedt</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Hood Canal Coordinating Council</strong></p></td>
					<td valign="top"><p>Hood Canal Coordinating Council</p></td>
					<td valign="top"><p><strong>Scott Brewer</strong></p></td>
					<td valign="top"><p>Alicia Olivas</p></td>
					<td valign="top"><p>Haley Harguth</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Island Watershed</strong></p></td>
					<td valign="top"><p>Island County</p></td>
					<td valign="top"><p><strong>Helen Price Johnson</strong></p></td>
					<td valign="top"><p>Dawn Pucci</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Nisqually Watershed</strong></p></td>
					<td valign="top"><p>Nisqually Tribe</p></td>
					<td valign="top"><p><strong>Chris Ellings</strong></p></td>
					<td valign="top"><p>Ashley Von Essen</p></td>
					<td valign="top"><p>Justin Hall</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Nooksack Watershed</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>NOPLE</strong></p></td>
					<td valign="top"><p>Jamestown S'Kallam Tribe and Clallam County </p></td>
					<td valign="top"><p><strong>Hansi Hals</strong></p></td>
					<td valign="top"><p>Mary Ellen Windborn</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Puyallup/Chambers Watershed</strong></p></td>
					<td valign="top"><p>Pierce County</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>Tom Kantz</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>San Juan Watershed</strong></p></td>
					<td valign="top"><p>San Juan County</p></td>
					<td valign="top"><p><strong>Jamie Stephens</strong></p></td>
					<td valign="top"><p>Byron Rot</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Skagit Watershed</strong></p></td>
					<td valign="top"><p>Skagit Watershed Council</p></td>
					<td valign="top"><p><strong>Richard Brocksmith</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Snohomish Watershed</strong></p></td>
					<td valign="top"><p>Sound Salmon Solutions and Snohomish County</p></td>
					<td valign="top"><p><strong>Rodney Pond</strong></p></td>
					<td valign="top"><p>Keith Binkley</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>South Sound Watershed</strong></p></td>
					<td valign="top"><p>Thurston County</p></td>
					<td valign="top"><p><strong>Cindy Wilson</strong></p></td>
					<td valign="top"><p>Amy Hatch-Winecka</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Stillaguamish Watershed</strong></p></td>
					<td valign="top"><p>City of Arlington </p></td>
					<td valign="top"><p><strong>Bill Blake</strong></p></td>
					<td valign="top"><p>Pat Stevenson</p></td>
					<td valign="top"><p>Donald (Kit) Crump</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>West Sound Watersheds</strong></p></td>
					<td valign="top"><p>Suquamish Tribe</p></td>
					<td valign="top"><p><strong>Rob Purser</strong></p></td>
					<td valign="top"><p>Tom Ostrom</p></td>
					<td valign="top"><p>Kathy Peters</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Lake WA/Cedar/Sammamish Watershed</strong></p></td>
					<td valign="top"><p>City of Edmonds and WRIA 8 Forum</p></td>
					<td valign="top"><p><strong>Diane Buckshnis</strong></p></td>
					<td valign="top"><p>Jason Mulvihill-Kuntz<strong></strong></p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Green/Duwamish Watershed</strong></p></td>
					<td valign="top"><p>City of Tukwila Council</p></td>
					<td valign="top"><p><strong>Dennis Robertson</strong></p></td>
					<td valign="top"><p>Doug Osterman</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>RECREATIONAL FISHING</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong>Puget Sound Anglers</p></td>
					<td valign="top"><p><strong>Ryley Fee</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>BUSINESS   &amp; AGRICULTURE</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Agriculture</strong></p></td>
					<td valign="top"><p>Skagitonians to Preserve Farmland</p></td>
					<td valign="top"><p><strong>Allen Rozema</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Business</strong></p></td>
					<td valign="top"><p>Washington Forest Protection Association</p></td>
					<td valign="top"><p><strong>Jason Callahan</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Business</strong></p></td>
					<td valign="top"><p>Washington Farm Bureaus</p></td>
					<td valign="top"><p><strong>Evans Sheffels</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Business</strong></p></td>
					<td valign="top"><p>Washington Policy Center</p></td>
					<td valign="top"><p><strong>Todd Myers</strong></p></td>
					<td valign="top"><p>Don Davidson</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>ENVIRONMENT</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Environment</strong></p></td>
					<td valign="top"><p>Long Live the Kings</p></td>
					<td valign="top"><p><strong>Jacques White</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Environment</strong></p></td>
					<td valign="top"><p>Washington Environmental Council </p></td>
					<td valign="top"><p><strong>Mindy Roberts</strong></p></td>
					<td valign="top"><p>David Burger</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Environment</strong></p></td>
					<td valign="top"><p>League of Women's Voters</p></td>
					<td valign="top"><p><strong>Ann Aagaard</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>FEDERAL   GOVERNMENT</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>EPA</strong></p></td>
					<td valign="top"><p>EPA</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>Peter Murchie</p></td>
					<td valign="top"><p>Angela Adams</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>USFWS</strong></p></td>
					<td valign="top"><p>USFWS</p></td>
					<td valign="top"><p><strong>Mary Root</strong></p></td>
					<td valign="top"><p>Molly Good<strong></strong></p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>NOAA</strong></p></td>
					<td valign="top"><p>NOAA</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>Elizabeth Babcock</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>USACOE</strong></p></td>
					<td valign="top"><p>USACOE</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>Evan Lewis</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>USDA/NRCS</strong></p></td>
					<td valign="top"><p>USDA/NRCS</p></td>
					<td valign="top"><p><strong>Roylens Rides at the Door</strong></p></td>
					<td valign="top"><p>Robin Slate</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>TRIBAL</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Lummi Nation</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>Alan Chapman</p></td>
					<td valign="top"><p>Lisa Wilson</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Makah Tribe</p></td>
					<td valign="top"><p><strong>Stephanie Martin</strong></p></td>
					<td valign="top"><p>Dana Sarff</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Nisqually Tribe</p></td>
					<td valign="top"><p><strong>David Troutt</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Nooksack Tribe</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Northwest Indian Fisheries Commission</p></td>
					<td valign="top"><p><strong>Justin Parker</strong></p></td>
					<td valign="top">Celilia Gobin</td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Point No Point Treaty Council</p></td>
					<td valign="top"><p><strong>Cynthia Rossi</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Port Gamble S'Klallam Tribe</p></td>
					<td valign="top"><p><strong>Paul McCollum</strong></p></td>
					<td valign="top"><p>Abby Welch</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Skokomish Tribe</p></td>
					<td valign="top"><p><strong>Dave Herrera</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Stillaguamish Tribe</p></td>
					<td valign="top"><p><strong>Pat Stevenson</strong></p></td>
					<td valign="top"></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
				  <td valign="top"><p>Swinomish &amp; Sauk-Suiattle</p></td>
					<td valign="top"><p><strong>Steve Hinton</strong></p></td>
				  <td valign="top">&nbsp;</td>
				  <td valign="top"></td>
			  </tr>
				<tr>
					<td valign="top"><p><strong>Tribal</strong></p></td>
					<td valign="top"><p>Tulalip Tribe</p></td>
					<td valign="top"><p><strong>Terry Williams</strong></p></td>
					<td valign="top"><p>Morgan Ruff</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>STATE   GOVERNMENT</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p><strong>&nbsp;</strong></p></td>
					<td valign="top"><p>&nbsp;</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>WDNR</strong></p></td>
					<td valign="top"><p>WDNR</p></td>
					<td valign="top"><p><strong>Hilary Franz</strong></p></td>
					<td valign="top"><p>Kirsten Feifel</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>Ecology</strong></p></td>
					<td valign="top"><p>Ecology</p></td>
					<td valign="top"><p><strong>Maia Bellon</strong></p></td>
					<td valign="top"><p>Joe Burcar</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>WDFW</strong></p></td>
					<td valign="top"><p>WDFW</p></td>
					<td valign="top"><p><strong>Vacant</strong></p></td>
					<td valign="top"><p>Justin Allergo</p></td>
					<td valign="top"><p>&nbsp;</p></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>WSCC</strong></p></td>
					<td valign="top"><p>WSCC</p></td>
					<td valign="top"><p><strong>Mark Clark</strong></p></td>
					<td valign="top"><p>Ron Shultz</p></td>
					<td valign="top"></td>
				</tr>
				<tr>
					<td valign="top"><p><strong>PSP</strong></p></td>
					<td valign="top"><p>PSP</p></td>
					<td valign="top"><p><strong>Laura Blackmore</strong></p></td>
					<td valign="top"><p>Sheida Sahandy</p></td>
					<td valign="top"><p>Jay Manning</p></td>
				</tr>
			</table>
			<!-- InstanceEndEditable --> </div>
		
		<div class="col-sm-2 padding-20-top padding-0-right"> <!-- InstanceBeginRepeat name="right_nav_repeat" --><!-- InstanceBeginRepeatEntry --> <!-- InstanceBeginEditable name="right_nav_title" -->
			<div class="right-nav-title margin-0-top ">BOARD CONTACT</div>
			<!-- InstanceEndEditable -->
			<ul class="nav-rightside-custom">
				<!-- InstanceBeginEditable name="right_nav_links" -->
					<li>
					
					<?php include 'includes/board_contact.html';?>
					
					
				</li>
					<li><a href="https://pspwa.box.com/s/gabtcrbzo9i5yybkeyi6lx6cez0bh10o"> Board's documents, meeting materials, summaries, and Resolutions</a></li>
				<li><a href="https://pspwa.box.com/s/gabtcrbzo9i5yybkeyi6lx6cez0bh10o">Board's 2018 meeting calendar</a></li>
				<li><a href="https://pspwa.box.com/s/87rfvgjwf6wahymd806v7lrkh0a89zjt">Puget Sound Environmental Caucus members </a></li>
				<!-- InstanceEndEditable -->
			</ul>
			<!-- InstanceEndRepeatEntry --><!-- InstanceEndRepeat --> </div>
	</div>
	<!--END OF ROW --> 
</div>
<!--END OF CONTENT CONTAINER -->

<?php include 'includes/footer-inc.html';?>

<!-- jQuery (necessary for Bootstrap's JavaScript plugins) --> 
<script src="js/jquery-1.11.2.min.js"></script> 

<!-- Include all compiled plugins (below), or include individual files as needed --> 
<script src="js/bootstrap.js"></script> 
<!-- custom js --> 
<script src="js/custom.js"></script> 
<!-- Google Tracking  --> 
<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-69373425-1', 'auto');
  ga('send', 'pageview');

</script>
</body>
<!-- InstanceEnd --></html>
